...
public class SimpleJPQLTests {
    ...
    /* 필기.
    *   JPQL 은 엔티티 갯체를 중심으로 개발할 수 있는
    *   객체 지향 쿼리이다. SQL 보다 간결하며 특정 DBMS 에 의존!!! 하지 않는다.
    *   find() 메소드를 통한 조회와는 다르게, 항상 데이터베이스에 SQL 을 실행해서 결과를
    *   조회한다.
    *  */
    ...
    *   - Criteria Query    : JPQL 을 평하게 작성하도록 도와주는 API
    *   */
    ...
    *   - 2-1. TypedQuery : 반환할 타입을 명확하게 지정하는 방식 일때 사용하며 쿼리 객체의 메소드
    *                       실행 결과로 타입이 반환된다.
    ...
    *  */
    ...
    @DisplayName("TypedQuery 를 이용한 단일 메뉴(단일행, 단일 컬럼) 조회 테스트")
    @Test
...
//        menuList.forEach(menu -> System.out.println(menu));
    ...
    void testDistinct(){
        ...
        // int -> Interger, char, short, String
