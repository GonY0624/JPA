...
public class GroupTests {
    ...
    *   단, 몇가지의 주의사항이 존재한다.
    *   1. 그룹합수의 반환타입은 결과 값이 정수이면 Long, 실수이면 Double 로 반환된다.
    *   2. 값이 없는 상태에서 count 를 제외한 그룹합수는 null 이 되고, count 만 0 이 된다.
    *   -> 따라서 반환 값을 담기 우해 선언하는 변수 타입을 기본 자료형으로 하면
    *   -> 조회 결과를 언박싱 할 떄 nullpointinterexception 이 발생한다.
    ...
    *   select category_code, count(*)from tbl_menu 카테도리 코드를 기준으로 그룹화한다.
    ...
    *  */
    ...
    @DisplayName("COUNT 를 제외한 다름 그룹 함수의 조회결과가 없는 경우 테스트")
    @Test
